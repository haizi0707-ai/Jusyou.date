...
予想用CSV、またはTARGET出力HTMLを画面からアップロードします。
例: おためし.csv / ビク.html
"""
...
def extract_horse_headers_from_target_html(text: str) -> list[dict]:
    ...
    for hr in body.find_all("hr"):
        ...
        # 例: カピリナ 牝 5歳 横山典弘(58歳) 56 (美)田島俊明(52歳)
        jockey = ""
        trainer = ""
        weight = np.nan
...
def read_target_html_prediction(uploaded_file, race_meta: dict) -> pd.DataFrame:
    ...
    if out.empty:
        return out
    # 型補正
    for c in ["R", "距離", "頭数", "馬番", "前走頭数", "前4角", "前走RPCI", "前走PCI", "前走PCI3", "前走Ave3F", "前走上3F地点差", "前走上3F"]:
        if c in out.columns:
...
st.title("新ペースロジック判定アプリ vHTML対応")
st.caption("更新版：予想用アップロードは CSV / HTML / HTM / TXT すべて選択できます。iPhoneでHTMLが選べない場合も対応。")
...
st.subheader("入力ファイル")
st.info("予想用アップロード欄が『CSV / TARGET HTML』表示になっていれば最新版です。古い表示のままならGitHub/Streamlitがまだ更新前のapp.pyを読んでいます。")
...
with col1:
    pred_file = st.file_uploader(
        ...
        type=None,
        key="pred_anyfile",
        help="iPhoneでHTMLが選べない場合があるため、ファイル種別制限を外しています。CSVもHTMLもこの欄から選択できます。",
    )
with col2:
...
st.caption("TARGET HTMLを使う場合は、HTML内に今回の競馬場・距離・馬場が無いことがあるため、ここで指定します。CSVの場合はCSV内の列を優先します。")
